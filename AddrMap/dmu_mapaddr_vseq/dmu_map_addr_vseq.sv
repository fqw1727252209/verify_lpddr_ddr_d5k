class dmu_strip_addr_vseq extends dmu_base_vseq;

  `uvm_object_utils(dmu_strip_addr_vseq)
  `uvm_declare_p_sequencer(my_vsqr)

  function new(string name="dmu_strip_addr_vseq");
    super.new(name);
  endfunction

  writeNoSnpPtlSeq    wr_nosnpptl_seq;
  writeNoSnpFullSeq   wr_nosnpfull_seq;
  readNoSnpSeq        rd_nosnp_seq;

  apb_strip_addr_seq            strip_addr_apb_seq;
      ...
      fork
        begin
          for(int j=0;j<in_cnt;j++)begin
            if (i<15) begin
              addr_gen = `DMU_BASE0_ADDR+addr_2n_gen(j*`TB_ADDR_WIDTH'h40, strip_data[i]);
            end else begin
              addr_gen = `DMU_BASE0_ADDR+addr_3snf_gen(j*`TB_ADDR_WIDTH'h40, strip_data[i]);
            end
            $display("Send ADDR is %h", addr_gen);
            `uvm_do_on_with(wr_nosnpfull_seq,p_sequencer.chi_vsqr.Down_seqr_ch_[0],{
                            wr_nosnpfull_seq.NonSecure      == 0;
                            wr_nosnpfull_seq.CancelOnRetryAck == 1;
                            wr_nosnpfull_seq.Addr             == addr_gen;
                            wr_nosnpfull_seq.rand_dly_mode    == 0;})
          end
        ...
        begin
          ...
          for(int j=0;j<in_cnt;j++)begin
            ...
            $display("Send ADDR is %h", addr_gen);
            `uvm_do_on_with(wr_nosnpfull_seq,p_sequencer.chi_vsqr.Down_seqr_ch_[1],{
                            wr_nosnpfull_seq.NonSecure      == 0;
                            wr_nosnpfull_seq.CancelOnRetryAck == 1;
                            wr_nosnpfull_seq.Addr             == addr_gen;})
          end
...
class dmu_strip_rand_vseq extends dmu_base_vseq;

  `uvm_object_utils(dmu_strip_rand_vseq)
  `uvm_declare_p_sequencer(my_vsqr)

  function new(string name="dmu_strip_rand_vseq");
    super.new(name);
  endfunction
