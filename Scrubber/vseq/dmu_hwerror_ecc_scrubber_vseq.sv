class dmu_hwerror_ecc_scrubber_vseq extends dmu_base_vseq;
    `uvm_object_utils(dmu_hwerror_ecc_scrubber_vseq)
    `uvm_declare_p_sequencer(my_vsqr)

    function new(string name="dmu_hwerror_ecc_scrubber_vseq");
        super.new(name);
    endfunction

    `ifdef SIMU_DMU_APB_FTVIP
    ecc_vip_error_seq        ecc_enable_seq;
    ...
    virtual task body();
        ...
        `uvm_do_on_with(ecc_enable_seq,p_sequencer.apb_sqr_[0],{ecc_enable_seq.mode==0;ecc_enable_seq.ecc_mode==3;ecc_enable_seq.wrback==0;ecc_enable_seq.error_inject_trig==0;});
        ...
        `uvm_do_on_with(ecc_scrub_seq,p_sequencer.apb_sqr_[0],{ecc_scrub_seq.mode==0;ecc_scrub_seq.scrub_mode==3;start_addr==0;end_addr==50;});
        fork
            begin
        ...
        `uvm_info(get_full_name(), "Finish ecc enable set 10 1bit test...", UVM_LOW);
    ...
    virtual task post_body();
        ...
        `uvm_do_on_with(ras_apb_seq,p_sequencer.apb_sqr_[0],{ras_apb_seq.int_trig_mode==1'b1;ras_apb_seq.int_select=='h00600c00;});
        if(starting_phase) starting_phase.drop_objection(this);
    endtask
endclass